...
@pytest.mark.skip(
    reason="Chat scaffold has no tools; CI summarizer value is POST /summarize"
)
async def test_pass_at_k_tool_usage(
    run_eval: Any, eval_thresholds: dict[str, Any], score_collector: Any
) -> None:
    """Retained for parity with DB-memory eval layout; not applicable to this spike."""
    pytest.skip("Chat scaffold has no tools")
