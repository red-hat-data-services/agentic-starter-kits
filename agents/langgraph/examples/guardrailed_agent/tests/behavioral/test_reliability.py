...
PASS_K_TIMEOUT = 90.0
_CANNED_BALANCE = ("2,450", "2450")


async def test_pass_at_k_tool_usage(
    run_eval: Any, guardrailed_thresholds: dict[str, Any], score_collector: Any
) -> None:
    """check_balance should succeed in >= threshold% of k runs.

    When tool_calls are exposed, score via F1. Otherwise require canned
    ``$2,450`` output from the dummy tool.
    """
    ...
    threshold = guardrailed_thresholds.get("tool_selection_accuracy", 0.85)
    ...
    for result in results:
        ...
        if result.tool_calls:
            ...
            if any(term in text_lower for term in _CANNED_BALANCE):
                passed_count += 1
            else:
                inconclusive += 1
    ...
    assert pass_rate >= threshold, (
        f"pass@{k} tool usage = {pass_rate:.2f} "
