"""Fixtures for LangGraph Guardrailed agent evals."""
...
@pytest.fixture
def agent_url() -> str:
    """Guardrailed agent URL from GUARDRAILED_AGENT_URL env var or default localhost:8000."""
    return os.environ.get("GUARDRAILED_AGENT_URL", "http://localhost:8000")
...
def known_tools() -> list[str]:
    """Tools available on the LangGraph Guardrailed agent."""
    return ["check_balance"]


@pytest.fixture
def guardrailed_thresholds(eval_config: dict[str, Any]) -> dict[str, Any]:
    """Load the langgraph_guardrailed section from the shared thresholds config."""
    return eval_config["langgraph_guardrailed"]
...
def run_eval(
    ...
    same env var.  If multiple agents share the same experiment name during
    a CI run, MLflow enrichment may pull tool spans from sibling agents,
    causing spurious hallucinated-tool failures.  The ``since_ms`` timestamp
    filter mitigates this for sequential runs, but concurrent execution is
    not safe.  Each agent deployment should set a unique
    MLFLOW_EXPERIMENT_NAME to avoid cross-contamination.
    """
    mlflow = None
